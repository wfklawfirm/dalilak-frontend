...
const PLAN_STYLE: Record<string, React.CSSProperties> = {
  ...
  trial:     { background: 'rgba(107,39,55,0.1)', color: '#741622' },
  admin:     { background: 'rgba(107,39,55,0.15)', color: '#4a1020' },
  suspended: { background: '#F8EDEF', color: '#8F1D2C' },
...
const INP: React.CSSProperties = {
  width: '100%', padding: '9px 12px', border: '1.5px solid #E6E2DC', borderRadius: 10,
  fontSize: 13, fontFamily: "'Cairo','Inter',sans-serif", background: '#FAFAF8',
  color: '#191713', outline: 'none',
}
const LBL: React.CSSProperties = { fontSize: 12, fontWeight: 700, color: '#191713', display: 'block', marginBottom: 5 }
...
export default function AdminPage() {
  ...
  const SECTION: React.CSSProperties = {
    background: '#fff', borderRadius: 18, border: '1.5px solid #E6E2DC',
    boxShadow: '0 1px 4px rgba(0,0,0,0.04)', padding: 20,
  }

  return (
    <div dir={isAr ? 'rtl' : 'ltr'} style={{ minHeight: '100vh', background: '#F8F8F6', fontFamily: "'Cairo','Inter',sans-serif" }}>
      <style>{`
        * { box-sizing: border-box; }
        ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-thumb { background: #E6E2DC; border-radius: 4px; }
        ...
        .adm-filter-chip:hover { border-color: #8F1D2C !important; }
      ...
      {/* Header */}
      <header style={{ background: 'var(--header-gradient)', padding: '14px 24px', position: 'sticky', top: 0, zIndex: 50, boxShadow: '0 4px 24px rgba(80,10,10,0.28)', animation: 'admHeaderIn 0.3s cubic-bezier(0.22,1,0.36,1) both' }}>
      ...
      <main id="main-content" style={{ padding: '16px 24px 0', maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {TABS.map(t => (
            <button type="button" key={t.id}
              ...
              style={{
                ...
                background: tab === t.id ? 'linear-gradient(135deg, #741622, #8F1D2C)' : '#fff',
                color: tab === t.id ? '#fff' : '#69645C',
                boxShadow: tab === t.id ? '0 2px 8px rgba(143,29,44,0.25)' : '0 1px 3px rgba(0,0,0,0.06)',
                border: tab === t.id ? 'none' : '1px solid #E6E2DC',
          ...
          <Link href="/admin/content" style={{
            ...
            background: '#fff', color: '#741622', border: '1.5px dashed rgba(107,39,55,0.3)',
        ...
        {tab === 'stats' && stats && (
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px,1fr))', gap: 12 }}>
              {[
                { label: 'إجمالي المستخدمين', value: stats.total,           bg: '#fff',                 ic: <svg aria-hidden="true" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#69645C" strokeWidth="1.6"><path strokeLinecap="round" strokeLinejoin="round" d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path strokeLinecap="round" strokeLinejoin="round" d="M23 21v-2a4 4 0 00-3-3.87m-4-12a4 4 0 010 7.75"/></svg> },
                ...
                { label: 'تجريبي نشط',         value: stats.trial_active,    bg: 'rgba(107,39,55,0.06)', ic: <svg aria-hidden="true" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#741622" strokeWidth="1.6"><circle cx="12" cy="12" r="9"/><path strokeLinecap="round" d="M12 7v5l3 3"/></svg> },
                ...
                { label: 'معطّلون',            value: stats.suspended,       bg: '#F8EDEF',              ic: <svg aria-hidden="true" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#8F1D2C" strokeWidth="1.6"><circle cx="12" cy="12" r="9"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg> },
                { label: 'معدل التحويل',       value: stats.conversion_rate, bg: 'rgba(107,39,55,0.06)', ic: <svg aria-hidden="true" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#741622" strokeWidth="1.6"><path strokeLinecap="round" strokeLinejoin="round" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg> },
              ].map((s, i) => (
            ...
            <button type="button" onClick={() => { loadStats(); loadUsers() }}
              style={{ marginTop: 14, fontSize: 12, color: '#741622', background: 'none', border: 'none', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 4, fontFamily: 'inherit' }}>
                  ...
                  <tbody>
                    {filtered.map((u, i) => (
                      <tr key={u.username} className="adm-tr" style={{ borderBottom: i < filtered.length - 1 ? '1px solid #E6E2DC' : 'none', opacity: u.active ? 1 : 0.5, background: '#fff' }}>
                        ...
                        <td style={{ padding: '10px 14px' }}>
                          <span style={{ fontSize: 11, padding: '2px 9px', borderRadius: 99, fontWeight: 700, ...(PLAN_STYLE[u.status] || PLAN_STYLE[u.plan] || { background: '#E6E2DC', color: '#69645C' }) }}>
                        ...
                        <td style={{ padding: '10px 14px' }}>
                          <div style={{ display: 'flex', gap: 6 }}>
                            <button
                              ...
                              className="adm-btn"
                              style={{ fontSize: 11, background: 'rgba(107,39,55,0.08)', color: '#741622', padding: '4px 10px', borderRadius: 8, border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 700 }}>
                              {isAr ? 'تعديل' : 'Edit'}
                            </button>
                            {u.active && (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 520, overflowY: 'auto' }}>
                {feedback.map((f, i) => (
                  <div key={i} style={{ border: '1px solid #E6E2DC', borderRadius: 14, padding: 14 }}>
                    ...
                    <p style={{ fontSize: 12, color: '#69645C', margin: 0, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' as const }}>{f.answer}</p>
                  </div>
                ))}
              </div>
            )}
        ...
        {tab === 'gaps' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {gapStats && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px,1fr))', gap: 10 }}>
                {[
                  ...
                  { label: 'مفتوحة',        val: gapStats.open,         bg: '#F8EDEF' },
              ...
              {[
                ...
                <button type="button" key={f.k}
                  ...
                  style={{
                    ...
                    background: gapFilter === f.k ? 'linear-gradient(135deg, #741622, #8F1D2C)' : '#fff',
                    color: gapFilter === f.k ? '#fff' : '#69645C',
                    boxShadow: gapFilter === f.k ? '0 2px 8px rgba(143,29,44,0.2)' : '0 1px 3px rgba(0,0,0,0.04)',
                    outline: gapFilter === f.k ? 'none' : '1px solid #E6E2DC',
                  }}>
            ...
            {loading ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {contentGaps.map(gap => (
                  <div key={gap.id} className="adm-gap-row" style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E6E2DC', padding: 14 }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 8, gap: 10 }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: 13, fontWeight: 600, color: '#191713', margin: '0 0 6px', lineHeight: 1.4 }}>{gap.user_question}</p>
                        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                          <span style={{ fontSize: 10, color: '#8F1D2C', background: '#F8EDEF', padding: '1px 7px', borderRadius: 8, fontWeight: 600 }}>{gap.gap_type}</span>
                          {gap.priority === 'high' && (
                          ...
                          {gap.detected_procedure && (
                            <span style={{ fontSize: 10, color: '#69645C', background: '#E6E2DC', padding: '1px 7px', borderRadius: 8 }}>{gap.detected_procedure}</span>
                          )}
                        </div>
                      </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 10, paddingTop: 4 }}>
                <button
                  ...
                  disabled={loading}
                  style={{ flex: 1, padding: '10px 0', background: 'linear-gradient(135deg, #8F1D2C, #741622)', color: '#fff', borderRadius: 12, border: 'none', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', opacity: loading ? 0.6 : 1 }}
                >
                ...
                <button
                  ...
                  style={{ padding: '10px 18px', background: '#E6E2DC', color: '#69645C', borderRadius: 12, border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
                >
                  {isAr ? 'إلغاء' : 'Cancel'}
                </button>
              </div>
