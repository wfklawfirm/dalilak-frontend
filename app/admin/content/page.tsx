...
const STATUS_CONFIG: Record<string, { label: string; style: React.CSSProperties; next: string[] }> = {
  draft:     { label: 'مسودة',    style: { background: '#E6E2DC', color: '#69645C' },          next: ['review'] },
  review:    { label: 'مراجعة',   style: { background: '#FFFBEB', color: '#B8860B' },           next: ['approved', 'draft'] },
  approved:  { label: 'معتمد',    style: { background: 'rgba(107,39,55,0.1)', color: '#741622' }, next: ['published', 'review'] },
  published: { label: 'منشور',    style: { background: '#FFFBEB', color: '#78350F' },           next: ['expired'] },
  expired:   { label: 'منتهي',    style: { background: '#F8EDEF', color: '#8F1D2C' },           next: [] },
}

const TRANSITION_CONFIG: Record<string, { label: string; bg: string; color: string }> = {
  review:    { label: 'إرسال للمراجعة', bg: '#B8860B', color: '#fff' },
  approved:  { label: 'اعتماد',         bg: '#741622', color: '#fff' },
  draft:     { label: 'إرجاع للمسودة',  bg: '#E6E2DC', color: '#69645C' },
  published: { label: 'نشر',            bg: '#78350F', color: '#fff' },
  expired:   { label: 'تحديد كمنتهي',  bg: '#8F1D2C', color: '#fff' },
}
    ...
    <div dir={isAr ? 'rtl' : 'ltr'} style={{ minHeight: '100vh', background: '#F8F8F6', fontFamily: "'Cairo','Inter',sans-serif" }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '20px 20px 60px' }}>
        ...
        <div className="pipeline-grid" style={{ marginBottom: 20 }}>
          {Object.entries(STATUS_CONFIG).map(([status, cfg], i) => (
            <button
              ...
              style={{
                padding: '14px 10px', borderRadius: 14, border: filterStatus === status ? '2px solid #8F1D2C' : '1.5px solid #E6E2DC',
                background: filterStatus === status ? '#FEF7F7' : '#fff',
                boxShadow: filterStatus === status ? '0 2px 12px rgba(143,29,44,0.12)' : '0 1px 3px rgba(0,0,0,0.04)',
            ...
            >
              ...
              <div style={{ fontSize: 24, fontWeight: 900, color: '#191713' }}>{statusCounts[status] || 0}</div>
            </button>
          ))}
        </div>
        ...
        <main id="main-content" className="content-main" style={{ gridTemplateColumns: selected ? '1fr 1fr' : '1fr' }}>
          ...
          <div>
            {loading ? (
              ...
              <div style={{ background: '#F8EDEF', border: '1px solid rgba(143,29,44,0.2)', borderRadius: 14, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: 13, color: '#8F1D2C', fontWeight: 600, flex: 1 }}>{loadError}</span>
                <button type="button" onClick={loadItems} style={{ fontSize: 11, fontWeight: 700, color: '#8F1D2C', background: 'none', border: '1px solid rgba(143,29,44,0.3)', borderRadius: 8, padding: '3px 10px', cursor: 'pointer', fontFamily: 'inherit' }}>{isAr ? 'إعادة المحاولة' : 'Retry'}</button>
              </div>
            ) : filteredItems.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '48px 20px', background: '#fff', borderRadius: 18, border: '1.5px solid #E6E2DC' }}>
                ...
                {filteredItems.map((item, i) => (
                  <button
                    ...
                    style={{
                      ...
                      border: selected?.id === item.id ? '2px solid #8F1D2C' : '1.5px solid #E6E2DC',
                      background: selected?.id === item.id ? '#FEF7F7' : '#fff',
                      boxShadow: selected?.id === item.id ? '0 2px 10px rgba(143,29,44,0.12)' : '0 1px 3px rgba(0,0,0,0.04)',
          ...
          {/* Detail panel */}
          {selected && (
            <div key={selected.id} style={{ background: '#fff', borderRadius: 18, border: '1.5px solid #E6E2DC', boxShadow: '0 2px 12px rgba(0,0,0,0.05)', height: 'fit-content', position: 'sticky', top: 76, animation: 'cgDetailIn 0.28s cubic-bezier(0.22,1,0.36,1) both' }}>
              {/* Detail header */}
              ...
              {STATUS_CONFIG[selected.status]?.next.length > 0 && (
                <div style={{ padding: '16px 20px', borderBottom: '1px solid #E6E2DC' }}>
                  ...
                  <textarea
                    ...
                    style={{ width: '100%', padding: '8px 12px', border: '1.5px solid #E6E2DC', borderRadius: 10, fontSize: 12, resize: 'none', marginBottom: 10, outline: 'none', fontFamily: 'inherit', color: '#191713', background: '#FAFAF8', transition: 'border-color 0.18s, box-shadow 0.18s' }}
                  />
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {STATUS_CONFIG[selected.status].next.map(target => {
                      const t = TRANSITION_CONFIG[target] || { label: target, bg: '#E6E2DC', color: '#69645C' }
                      return (
                        <button
                          type="button"
                          key={target}
            ...
            <div role="dialog" aria-modal="true" aria-label={isAr ? 'إنشاء محتوى جديد' : 'Create new content'} onKeyDown={e => { if (e.key === 'Escape') setShowCreate(false) }} style={{ background: '#fff', borderRadius: 20, width: '100%', maxWidth: 500, padding: 24, boxShadow: '0 20px 60px rgba(0,0,0,0.15)', animation: 'cgModalIn 0.3s cubic-bezier(0.22,1,0.36,1) both' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <button
                  ...
                  style={{ padding: '11px', borderRadius: 12, background: 'linear-gradient(135deg, #741622, #8F1D2C)', border: 'none', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', opacity: creating ? 0.7 : 1 }}
                >
