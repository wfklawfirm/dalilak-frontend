  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8F8F6', fontFamily: "'Cairo','Inter',sans-serif" }} dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <main id="main-content" style={{ maxWidth: 'var(--container-md)', margin: '0 auto', padding: '16px 14px var(--bottom-nav-clearance)' }}>
        ...
        {/* batch #498 declutter: this row used to carry a second "Home" button
            next to the primary CTA — but it navigated to '/', the exact same
            destination as the BottomNav's Home tab rendered a few hundred
            pixels below on every viewport. Two controls, one destination, no
            reason to keep both above the fold. One clean full-width primary
            CTA now — nothing users could reach before is now unreachable. */}
        <div style={{ marginBottom: 20 }}>
          <button type="button" aria-label={isAr ? 'اسأل دليلك عن هذه المعاملة' : 'Ask Dalilak about this procedure'} onClick={askAI} className="btn-primary" style={{ width: '100%', padding: '12px', background: 'var(--brand)', color: '#fff', border: 'none', borderRadius: 12, fontFamily: 'inherit', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
