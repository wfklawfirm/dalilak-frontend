      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div>
          <label htmlFor="rp-token" style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#5C4A3A', marginBottom: 5 }}>
            {isAr ? 'رمز الاستعادة' : 'Recovery code'} <span style={{ color: '#8B1A1A' }}>*</span>
          </label>
          <input
            id="rp-token"
            type="text"
        ...
        <div>
          <label htmlFor="rp-password" style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#5C4A3A', marginBottom: 5 }}>
            {isAr ? 'كلمة المرور الجديدة' : 'New password'} <span style={{ color: '#8B1A1A' }}>*</span>
          </label>
  ...
  return (
    <div id="main-content" dir={isAr ? 'rtl' : 'ltr'} style={{
      ...
      <div style={{
        ...
        <Suspense fallback={<div style={{ textAlign: 'center', padding: 24, color: '#9C8E80', fontSize: 13 }}>{isAr ? 'جاري التحميل...' : 'Loading...'}</div>}>
          <ResetForm />
        </Suspense>
      </div>
