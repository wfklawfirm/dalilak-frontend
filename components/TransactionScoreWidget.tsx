...
export default function TransactionScoreWidget({ score, isAr, onAction }: Props) {
  const scoreColor = score.score >= 80 ? '#78350F' : score.score >= 50 ? '#B8860B' : '#8B1A1A'
    ...
    <div style={{
      ...
      <div style={{ height: 6, background: '#EAE4D9', borderRadius: 3, overflow: 'hidden', marginBottom: 14 }}>
        <div style={{
          ...
          boxShadow: `0 0 6px ${score.score >= 80 ? 'rgba(180,83,9,0.35)' : score.score >= 50 ? 'rgba(184,134,11,0.3)' : 'rgba(139,26,26,0.3)'}`,
        }} />
      </div>
        ...
        {breakdownItems.map((item, i) => (
          <div key={item.key} style={{ animation: 'tswItem 0.18s cubic-bezier(0.22,1,0.36,1) both', animationDelay: `${i * 0.06}s` }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
              ...
              <span style={{ fontSize: 10.5, fontWeight: 700, color: item.value >= 70 ? '#78350F' : item.value >= 40 ? '#B8860B' : '#8B1A1A' }}>{item.value}%</span>
            </div>
      ...
      {score.blockingIssues.length > 0 && (
        <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 10, padding: '10px 12px', marginBottom: 12 }}>
          <p style={{ fontSize: 10.5, fontWeight: 700, color: '#8B1A1A', margin: '0 0 6px' }}>
