...
#   lib/archiveDocuments.ts   — 4378 وثيقة حكومية حقيقية (نص + عنوان + جهة
#                               + سنة + رابط PDF)، مستخرجة ومصنّفة من أرشيف
#                               6504 ملف (المجلدات 05+06+07)، بعد استبعاد
#                               353 ملف أرشيف زحف تالف، 122 ملف داخلي لجامعة
#                               AUB، 1474 ملف تعذّر استخراج نص منه، و177 نسخة
#                               مكررة حرفياً. راجع forms_pipeline/ في مجلد
#                               المشروع الرئيسي لتفاصيل خط الاستخراج الكامل.
#   app/archive/page.tsx      — صفحة "أرشيف الوثائق الرسمية": بحث + تصفية
#                               حسب الجهة (18 جهة) + بطاقات بنفس نمط /forms
#   app/archive/layout.tsx    — metadata/SEO لصفحة /archive
#   components/MobileMenu.tsx — أُضيف رابط "أرشيف الوثائق الرسمية" لقائمة
#                               التنقّل الجانبية
#
#   ملاحظة: ملفات PDF الأصلية (١٫٨GB) مستضافة على مستودع منفصل
#   github.com/wfklawfirm/dalilak-forms-archive (راجع
#   forms_pipeline/archive_staging/push_archive.sh لرفعها — يتطلب إنشاء
#   المستودع الفارغ على GitHub أولاً). حتى يتم رفعه، روابط "عرض الملف
#   الأصلي" في صفحة /archive ستُرجع 404.
#
#   نفس بيانات هذا الأرشيف مرفوعة بالكامل إلى Qdrant (collection
#   dalilak_ai_v2، domain=official_documents_archive) لإغناء إجابات الشات
#   بوت — عملية منفصلة عن الفرونتند، لم تُغيّر أي Backend API.
git add lib/archiveDocuments.ts app/archive/page.tsx app/archive/layout.tsx components/MobileMenu.tsx
git diff --cached --quiet || git commit -m "feat: أرشيف الوثائق الرسمية — 4378 وثيقة حكومية حقيقية (صفحة /archive + بيانات + رابط تنقّل)، مرفوعة أيضاً لقاعدة Qdrant"
