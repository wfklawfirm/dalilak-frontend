...
# ================================================================
# Batch #503 — بنك الخدمات الحقيقي (368) أصبح قابلاً للتصفّح ضمن /procedures
#   طلب المستخدم بعد رؤية "98 إجراء موثّق" فقط في شريط الإحصائيات رغم وجود
#   آلاف المعاملات: "قم بالتدقيق و إضافة كل الاجراءات الموثقة المتاحة".
#   التدقيق (بقراءة الكود فعلياً، لا بالتخمين) كشف:
#   - 98 = PROCEDURES_DATA النشطة (27) + ENRICHED_PROCEDURES (71) فقط —
#     وهذه فعلاً كل الإجراءات ذات الخطوات المرقّمة الكاملة.
#   - 368 = ALL_SERVICES (lib/allServices.ts) — بيانات حقيقية 100% موثّقة
#     أصلاً (رسوم/مستندات مطلوبة/هاتف/موقع/ساعات عمل/نماذج/ملاحظات لكل
#     خدمة، مستخرجة من مصادر رسمية حقيقية — لا Mock Data) لكنها كانت
#     مُستخدَمة فقط لحساب رقم "368 خدمة متاحة" في الإحصائيات، ولا تظهر
#     إطلاقاً ضمن قائمة البحث/الفلترة القابلة للتصفّح في نفس الصفحة —
#     كانت تظهر فقط عبر بطاقة رابط صغيرة أسفل الصفحة تُحوِّل لصفحة
#     /services منفصلة. لا بيانات مفقودة أو تحتاج اختراعاً — فقط بيانات
#     حقيقية موجودة أصلاً ولم تكن معروضة في المكان الذي وعد المستخدم بها.
#   الحل (frontend بالكامل، صفر بيانات جديدة أو مُخترَعة):
#   - app/procedures/page.tsx: أُضيف filteredServices (useMemo) يُطبّق نفس
#     منطق البحث/فلتر الوزارة/الفلاتر المتقدمة على ALL_SERVICES، ويُعرَض
#     كقسم ثالث "خدمات إضافية" ضمن نفس قائمة النتائج (بطاقة قابلة للتوسيع
#     تعرض الوصف/الرسوم/مدة الإنجاز/الوثائق المطلوبة/الهاتف والموقع
#     وساعات العمل/النماذج/الملاحظات المهمة — كل حقل من allServices.ts
#     نفسه، بلا أي إضافة). PROCEDURES_TOTAL أصبح 98+368=466 بدل 98 فقط،
#     فرأس الصفحة الآن يَعِد بما هو فعلاً قابل للتصفّح أسفله مباشرة.
#   - فلتر "بدأت هذه المعاملة بالفعل" يُصرَّح بصراحة بلا نتائج لهذه
#     الخدمات (لا آلية تتبّع بدء حقيقية لها) بدل اختراع حالة غير موجودة.
git add app/procedures/page.tsx
git diff --cached --quiet || git commit -m "feat: batch #503 — إظهار بنك الخدمات الحقيقي (368) ضمن قائمة /procedures القابلة للتصفّح، بدل حصره في عدّاد إحصائي فقط"

# ================================================================
# Batch #504 — إدراج المعاملات الناقصة من بنك الأرشيف الخام (2,484) في /procedures
#   طلب المستخدم بعد batch #503: "دقق اقصد و ادرج المعاملات الناقصة و
#   الموجودة في الارشيف" — أي استكمال نفس منطق batch #503 لبنك المعاملات
#   الخام الثالث (lib/allTransactions.ts، TX_ALL، 2,484 معاملة حقيقية من
#   أرشيف dawlati.gov.lb، كانت مقروءة فقط عبر تبويب "معاملات" في /forms،
#   ولا تظهر إطلاقاً في /procedures).
#   الفرق عن batch #503: هذا المصدر (عنوان+وزارة+رابط PDF فقط، بلا وصف/
#   رسوم/خطوات) يتقاطع جزئياً مع الـ 466 الموثّقة أصلاً (نفس المعاملة
#   بعنوانين مختلفين قليلاً من مصادر Scrape مختلفة) — فبُني فهرس مطابقة
#   عناوين (COVERED_TITLES / isTxAlreadyDocumented) يُقارن عنوان كل معاملة
#   خام بعناوين PROCEDURES_DATA + ENRICHED_PROCEDURES + ALL_SERVICES
#   (مطابقة جزئية بالاحتواء، بحد أدنى 6 أحرف لتجنّب تطابقات عشوائية قصيرة)
#   ويُستبعَد كل ما هو مُغطّى أصلاً — فلا تكرار، وتُعرَض فقط المعاملات
#   "الناقصة" فعلياً بالمعنى الحرفي لطلب المستخدم.
#   - lib/allTransactions.ts (~400KB) يُحمَّل بـ dynamic import بعد
#     التركيب (لا استيراد ثابت) تماشياً مع batch #354/#497 — لا يُبطّئ
#     الحمولة الأولى لأي زائر.
#   - المطابقة الثقيلة (2,484×~460 مقارنة) تُحسَب مرة واحدة فقط عند تحميل
#     البيانات (useMemo بلا اعتماد على search) — تصفية البحث/الوزارة بعدها
#     خفيفة وفورية مع كل ضغطة مفتاح.
#   - تُعرَض كقسم رابع "من أرشيف المعاملات الرسمي (غير مُصنَّفة بعد)" —
#     صفوف مبسّطة (عنوان + وزارة + رابط الوثيقة الرسمية إن وجد + زر اسأل
#     دليلك)، بلا تفاصيل مخترعة، مع ترقيم صفحات "عرض المزيد" (20 في كل
#     مرة) بدل عرض آلاف الصفوف دفعة واحدة.
#   - فلتر "بدأتها بالفعل" يُخفي هذا القسم بالكامل (نفس منطق batch #503 —
#     لا آلية تتبّع بدء حقيقية لهذا المصدر).
git add app/procedures/page.tsx
git diff --cached --quiet || git commit -m "feat: batch #504 — إدراج المعاملات الناقصة من بنك الأرشيف الخام (2,484) في /procedures، بلا تكرار مع الموثّق أصلاً"

# ================================================================
# Batch #505 — تصنيف وتحسين عناوين أرشيف الوثائق (5,234) عبر AI
#   طلب المستخدم: "قم بتصنيفها و تحسينها و تاكد من تحسين الكتابة في عناوين
#   الارشيف كلها كونه يوجد العديد من الملفات غير منظمة و غير مقروؤة و اجعل
#   الذكاء الاصطناعي اذا اضطر الامر يعيد تسميتها و تصنيفها".
#   تدقيق: ~45% من عناوين lib/archiveDocuments.ts (5,234 وثيقة) كانت بلا
#   معنى (فارغة/OCR فاشل)، والباقي فعلياً مجرد أول سطر مُستخرَج آلياً لا
#   عنوان حقيقي، ولا يوجد أي حقل تصنيف إطلاقاً. مرّت كل الوثائق عبر
#   gpt-4o-mini (نفس مفتاح OpenAI الموجود أصلاً في backend/.env، لا مفتاح
#   جديد) بتعليمات صارمة بمنع اختراع أي تفصيل غير مذكور في المقتطف/الجهة/
#   السنة، مع fallback صادق "وثيقة رسمية من {الجهة} ({السنة})" حين يتعذّر
#   الفهم. النتيجة: 0/5,234 عنوان غير مقروء متبقٍّ، وتصنيف حقيقي ضمن 10
#   أنواع وثائق ثابتة.
#   أمان: title الأصلي لم يُمَس إطلاقاً (لا يزال يطابق حرفياً sources
#   العائدة من الباك-إند/RAG — findArchiveDocByTitle يستمر بالعمل بلا أي
#   كسر). أُضيف حقل جديد فقط: displayTitle (العنوان المُحسَّن) وcategory.
#   العرض المرئي في /archive وProcedureDetailClient.tsx وArchiveAISearch.tsx
#   تحوّل لـ displayTitle؛ المطابقة الداخلية بقيت على title الأصلي.
git add lib/archiveDocuments.ts app/archive/page.tsx "app/procedures/[slug]/ProcedureDetailClient.tsx" components/ArchiveAISearch.tsx
git diff --cached --quiet || git commit -m "feat: batch #505 — تصنيف وتحسين عناوين أرشيف الوثائق (5,234) عبر AI، بلا كسر لمطابقة RAG"
